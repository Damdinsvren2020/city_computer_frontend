...
const Basket = ({ userDetail }) => {
  ...
  useEffect(() => {
    const getWishList = async () => {
      const { data } = await axios.get("/userCart/" + userDetail._id)
      if (data.success) {
  ...
  const gotoCard = async () => {
    if (wishlist.length !== 0) {
      ...
      Swal.fire({
        icon: "warning",
        title: "Та барааний сонголтоо хийгээрэй ☺️"
      })
      return navigate("/")
    }
  }
  ...
  return (
    <div className="p-2">
      <div class="p-4 w-[850] rounded-sm overflow-auto  h-full bg-white border border-gray-200 shadow-md dark:bg-gray-800 dark:border-gray-700">

        <div className="justify-center items-center flex w-[900] h-full overflow-auto">
          {
      ...
      <button onClick={() => gotoCard()} className="w-full bg-red-500 text-white p-2 rounded-sm mt-2 hover:bg-green-500">
        Худалдан авалт хийх
      </button>
    </div>
  );
