          ...
          wishlist.length !== 0 ? wishlist.map((item, index) => (
            <div className="w-48 justify-center items-center mx-2">
              ...
              <div className="w-full justify-evenly items-center flex mx-auto bg-gray-300 p-2">
                <button onClick={() => removeProduct(item?.product?._id)} className="w-8 h-8 rounded-full bg-red-500 text-lg text-white flex justify-center items-center" >
                  <h1>
                    <HiMinus />
                  </h1>
                </button>
                <h1 className="text-xl">
                  {item?.count[item?.product?._id]}
                </h1>
